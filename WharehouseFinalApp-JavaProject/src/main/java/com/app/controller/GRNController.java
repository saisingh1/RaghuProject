...
public class GRNController {
	...
	public String savePurchase(@ModelAttribute GoodRecieveNote goodRecieveNote,Errors errors,ModelMap map) {
		...
		if (errors.hasErrors()) {
			...
			Integer orderId = goodRecieveNote.getPurchase().getOrderId();
			Purchase purchase=purchaseService.getPurchaseById(orderId);
			purchase.setOrderStatus("RECEIVED");
	...
	private void getPurchaseDtls(Integer grnId,ModelMap map) {
		...
		map.addAttribute("orderId", goodRecieveNote.getPurchase().getOrderId());
		map.addAttribute("orderStatus", goodRecieveNote.getPurchase().getOrderStatus());
		...
		map.addAttribute("nullCount",purchaseService.getNullGrnStatusCount()+"");
