...
public class SaleOrderDaoImpl implements ISaleOrderDao {
	...
	public boolean isOrderCodeExist(String saleOrderCode) {
		...
		long count=0;
		String hql="select count(saleOrderCode) from "+SaleOrder.class.getName()+" where saleOrderCode=?0";
	...
	@SuppressWarnings("unchecked")
	public long getNullShippingStatusCount() {
		long count=0;

		DetachedCriteria hql=
				DetachedCriteria.forClass(SalesDetails.class)
				.setProjection(Projections.projectionList()
						.add(Projections.count("shipSatus"))
						)
				.add(Restrictions.eq("shipSatus",null));
